...
function getSubmissionsForAssignment($database, $id_class, $id_assignment){
    ...
    $query = "SELECT students.id, CONCAT_WS(' ',students.firstname, students.prefix, students.lastname) as name, DATE_FORMAT(submissions.time, '%d %M %Y, %H:%i') as submission_date, submissions.submission_count as submission_count
